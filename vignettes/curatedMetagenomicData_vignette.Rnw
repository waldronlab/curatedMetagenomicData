...
\Rfunction{listPackageResources} returns the names of all resources in 
the package:
<<example1loadtep2>>=
allData <- listPackageResources("curatedMetagenomicData")
length(allData)
head(allData)
@
...
<<example1loadtep2>>=
listPackageResources("curatedMetagenomicData", filterBy="throat")
@

The name of each resource can be invoked as a function. The \code{metadata}
argument controls whether metadata are returned or if the data are
loaded. 

\code{metadata=TRUE} returns just the metadata:
<<example1loadtep3, eval=TRUE>>=
WT2D.stool.abundance.eset.rda(metadata=TRUE)
@

\code{metadata=FALSE} loads the dataset:
<<example1loadtep4, eval=TRUE>>=
WT2D.stool.abundance.eset.rda(metadata=FALSE)
@

Data can also be loaded with the \code{loadPackageResources} function. The
\code{filterBy} argument can be the name of a resource or any term in the
resource metadata.
<<example1loadtep5, eval=TRUE>>=
loadPackageResources("curatedMetagenomicData", filterBy="WT2D.stool.abundance")
@
...
set of bodysites or studies we are interested in. \code{loadPackageResources} 
filters resources based on the values in \code{filterBy} then loads the 
datasets into a list.

Any term in the resource metadata can be used as a filter term:
<<>>=
t2dmeta_long.stool.pathcoverage.eset.rda(metadata=TRUE)
@

Body type, study and data type terms can be seen in the resource titles:
<<>>=
head(listPacakgeResources("curatedMetagenomicData"))
@

List all datasets that were collected only from body site \dQuote{saliva}:
<<>>=
filtered <- listPackageResources("curatedMetagenomicData", filterBy="saliva")
filtered
@ 

Load the data as a list of \Rclass{ExpressionSet} objects:
<<>>=
esets <- loadPackageResources("curatedMetagenomicData", filterBy=filtered)
sapply(esets, class)
@

We could have accomplished the same thing by passing \dQuote{saliva} as the
\code{filteredBy} argument:
<<eval=FALSE>>=
loadPackageResources("curatedMetagenomicData", filterBy="saliva")
@

Search terms can be combined for more detailed filtering. There are quite
a few datasets from the \dQuote{hmp} study:

<<example4loadHmpiistep1>>=
length(listPackageResources("curatedMetagenomicData", "hmp"))
@ 

We can further narrow the results by adding filtering by site 
\dQuote{hard_palate"}:
<<example4loadHmpiistep1>>=
listPackageResources("curatedMetagenomicData", c("hmp", "hard_palate"))
@ 

Load the resources into a list:
<<example4loadHmpiistep1>>=
hmp <- loadPackageResources("curatedMetagenomicData", 
                            c("hmp", "hard_palate", "marker"))
names(hmp)
@
...
esets <- loadPackageResources("curatedMetagenomicData", "\\.abundance")
