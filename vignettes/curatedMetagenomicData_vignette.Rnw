...
\section{Section 1} Human Microbiome has played a central role in clinical
research due to its impact on human health and diseases. Shotgun metagenomics
sequencing technology has provided a precise way to profile microbial diversity
from various human body sites across different individuals, enabling us to
better understand the relationship between specific microbiota to human health
and some diseases. To date, many human micro biome datasets have been generated
from numerous studies as well as consortiums such as Human microbiome project
...
Use the \Rfunction{availableResources} function to see a list of all datasets:
<<example1loadtep2, eval=FALSE>>=
avaialbleResources()
@

The name of each resource is a function that either lists the metadata or 
loads the data. Setting \code{metadata=TRUE} returns the metadata:
<<example1loadtep3, eval=TRUE>>=
WT2D.stool.abundance.eset(metadata=TRUE)
@

To load the WT2D.stool.abundance data use \code{metadata=FALSE}:
...
\section{Load datasets based on rules} For a meta-analysis, we typically want
to filter datasets and samples to get a set of bodysites or studies we are
interested in. We provide a helper function, \code{createResourcesList} that
filters the resources by bodytype and study and loads the data as a list of
\Rclass{ExpressionSet} objects.

Possible values for \code{bodytype} and \code{study} are taken from the
resource names:
<<example2loadstep1>>=
head(availableResources())
@

\subsection{Selecting only samples from stool}
To see a list of datasets that were collected only from body site 
``stool'', we invoke \code{filterResources}:

<<example3loadSkinstep1>>=
filtered <- filterResources(bodysite = "stool")
filtered
@ 

To load the data as a list of \Rclass{ExpressionSet} objects we can invoke
\code{createResourcesList} with the filtered resources:
<<example3loadSkinstep2>>=
createResourcesList(resources = filtered)
@

or simply specify the \code{bodysite} parameter to \code{createResourcesList}:
<<example3loadSkinstep3>>=
createResourcesList(bodysite = "stool")
@

\subsection{Selecting only datasets from hmpii}
Now, if we want to see what datasets were collected from study ``hmpii'':
<<example4loadHmpiistep1>>=
filterResources(study = "hmpii")
@ 

Load the hmpii datasets into a list of \code{eSet} obects:
<<example4loadHmpiistep2>>=
esets <- createResourcesList(study = "hmpii")
@

We have \Sexpr{length(esets)} datasets with samples that passed our filter
in a list of \Rclass{ExpressionSet} objects called \texttt{esets}:
...
abundance <- createResourcesList(study = "abundance")
