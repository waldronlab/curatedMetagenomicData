...
the package:
<<example1loadtep2>>=
allData <- listResources("curatedMetagenomicData")
length(allData)
head(allData)
@
...
<<example1loadtep2>>=
listResources("curatedMetagenomicData", filterBy="throat")
@
...
Data can also be loaded with the \code{loadResources} function. The
\code{filterBy} argument can be the name of a resource or any term in the
resource metadata.
<<example1loadtep5, eval=TRUE>>=
loadResources("curatedMetagenomicData", filterBy="WT2D.stool.abundance")
@
...
set of bodysites or studies we are interested in. \code{loadResources} 
...
<<>>=
head(listResources("curatedMetagenomicData"))
@

List all datasets that were collected only from body site \dQuote{saliva}:
<<>>=
filtered <- listResources("curatedMetagenomicData", filterBy="saliva")
filtered
@ 

Load the data as a list of \Rclass{ExpressionSet} objects:
<<>>=
esets <- loadResources("curatedMetagenomicData", filterBy=filtered)
sapply(esets, class)
@
...
<<eval=FALSE>>=
loadResources("curatedMetagenomicData", filterBy="saliva")
@

Search terms can be combined for more detailed filtering. Here we list all
datasets from the \dQuote{hmp} study:

<<example4loadHmpiistep1>>=
length(listResources("curatedMetagenomicData", "hmp"))
@ 

The results can be narrowed down by filtering by the body site 
\dQuote{hard_palate"}:
<<example4loadHmpiistep1>>=
listResources("curatedMetagenomicData", c("hmp", "hard_palate"))
@ 

Load the resources into a list:
<<example4loadHmpiistep1>>=
hmp <- loadResources("curatedMetagenomicData", c("hmp", "hard_palate"))
names(hmp)
@
...
esets <- loadResources("curatedMetagenomicData", "\\.abundance")
