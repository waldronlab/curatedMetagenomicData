...
#barplot
set.seed(14)
area_abundance <- ggplot(arrange(bardat, desc(OTU), Abundance), aes(x=Sample, y=Abundance, fill=OTU))+
    geom_area()+
...
box_alpha <- ggplot(alpha, aes(x=study, y=Shannon, fill=study)) +
  stat_boxplot(geom ='errorbar') +
  geom_boxplot() +
  theme(axis.text.x = element_blank()) +
  ylab("Shannon Alpha Diversity") + xlab("") 
box_alpha
```
...
main = cors$feature[cors$r.cor == max(cors$r.cor)] %>% 
  str_replace_all("\\.", " ") %>%
  str_replace_all("  ", " ")

par(mar=c(10,10,10,10))
scatter_pwy <- qplot(x    = eset_pathway$prev, 
     y    = exprs(eset_pathway)[cors$feature[cors$r.cor == max(cors$r.cor)]  , ], 
     main = main,
     ylab = "Pathway abundance",
     xlab = "Prevotella copri abundance", shape=I(1))
scatter_pwy
...
heat_pathway <- ggplot(data = melted_cors, aes(x=Var1, y=Var2, fill=value)) + 
    geom_tile() +
...
```{r max_2_cors}
max2cors <- arrange(melted_cors, desc(value))[1:2,]
max2cors$pvalue <- sapply(1:2, function(i) 
  cor.test(otu_table(subset_genus)[max2cors[i, 1] ,], 
           exprs_pwy[max2cors[i, 2]])
  )
```
...
svg("area_abundance.svg", width=8, height=6)
area_abundance
dev.off()

svg("box_alpha.svg", width=8, height=6)
box_alpha
dev.off()

svg("heat_pathway.svg", width=11, height=6)
heat_pathway
dev.off()

svg("scatter_pwy.svg", width=6, height=6)
scatter_pwy
dev.off()
