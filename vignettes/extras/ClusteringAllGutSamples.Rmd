...
plot_cluster_validation = function(bray,js,rjs,type=c("p","s","c"), legend=TRUE,...) {
  ...
  if(type=="p") {
    ...
    # no annotation for calinski
  }
}

```
```{r plot_Prediction_Strength}
plot_cluster_validation(ps_bray$mean.pred[2:10], ps_js$mean.pred[2:10], ps_rjs$mean.pred[2:10], ylab="Prediction Strength",ylim=c(0,1.1),legend=FALSE, type="p")
...
#plot silhouette widths
   plot_cluster_validation(si_bray,si_js,si_rjs, legend=FALSE, ylab="Average silhouette width", ylim=c(0,1), type="s")
...
plot_cluster_validation(ch_bray, ch_js, ch_rjs, legend=TRUE, ylab="Calinski-Harabasz score", ylim=c(0,300), type="c")
```
...
#Labels
p <- p + labs(x="Axis 1",y="Axis 2", 
              title=paste0("PCoA by selected abundances, displaying 2 clusters (n=", nrow(df_ordinate), ")"))
pcoa_clusters <- p
pcoa_clusters
...
p
## Without sizing
# p <- ggplot(data=df_ord_dataset_disease, aes(x=pc1, y=pc2,
#                                              shape=disease,
#                                              color=study)) 
# p <- p + geom_point()
# p <- p + labs(x="Axis 1", y="Axis 2", title="PCoA by dataset and disease")
...
```{r save_svg}
svg("pcoa_clusters.svg", width=8,height=6)
pcoa_clusters
dev.off()

svg_cv <- function(...) svg(..., width=6, height=5)


svg_cv("prediction_strength.svg")
  plot_cluster_validation(ps_bray$mean.pred[2:10], ps_js$mean.pred[2:10],
                          ps_rjs$mean.pred[2:10], ylab="Prediction Strength",
                          ylim=c(0,1.1),legend=FALSE, type="p")
dev.off()

svg_cv("calinksi.svg")
  plot_cluster_validation(ch_bray, ch_js, ch_rjs, legend=TRUE, 
                         ylab="Calinski-Harabasz score", ylim=c(0,300), type="c")
dev.off()

svg_cv("silhouette.svg") 
  plot_cluster_validation(si_bray,si_js,si_rjs, legend=FALSE, 
                          ylab="Average silhouette width", ylim=c(0,1), type="s")
dev.off()
