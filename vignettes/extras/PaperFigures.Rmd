...
Note, R must be started with the option `--max-ppsize=500000` for Figure 1 Example 1 and for Supplemental Figure 1 to run. Otherwise, it will generate an error `Error: protect(): protection stack overflow`. If you are running on a machine with limited memory or a 32-bit installation of R, or you do not want to wait overnight for these classification examples to run, change `eval=TRUE` to `eval=FALSE` in the chunk options for the chunks named `fig1eg` and `suppfig1`. One way to do this from the command line is to invoke R:
