# Script to read HUMAnN2 output file; e.g. Neilsen.tar.bz2
#Functions to be loaded before running the script given below (line 148)
...
curateNeilsonSampName <- function(sampName = sampName, phenotype.data=Dat, datasetName="Neilsen_genome_assembly"){
  dat_sel <- phenotype.data[phenotype.data$dataset_name == datasetName,]
  dat_sel_samp <- dat_sel[dat_sel$sampleID == sampName,]
  newSampName <- rownames(dat_sel_samp)
...
processTarBz2ByDatatype_batch <- function(tarbz2FileOutdir,
  ...
  for (i in 1:length(tsvfiles.sel)){
    ...
    #get sampName from directory name (this is to get correct sample names for the dataset)
    ...
    ## curate sample name for Neilson dataset as some names are the same as Chatelier_gut_obesity dataset
    sampName <- curateNeilsonSampName(sampName)
...
tarbz2FileOutdir <- "./Neilsen.tar.bz2"
